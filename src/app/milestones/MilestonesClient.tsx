...
export default function MilestonesClient() {
  ...
  if (milestonesError ?? progressError) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <EmptyState
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <EmptyState
          ...
          description="Check back later for new milestones to complete!"
        />
      </div>
    )
  }
  ...
  const userLevel = Math.floor(completedMilestones / 5) + 1
  const userXP = (completedMilestones % 5) * 400
  const maxXP = 2000
